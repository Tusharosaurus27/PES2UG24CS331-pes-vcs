...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    char path[512], dir[512], tmp[512], hex[HASH_HEX_SIZE + 1];
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    ObjectID check;
    compute_hash(buffer, file_len, &check);
    ...
    char *null_pos = memchr(buffer, '\0', file_len);
    if (!null_pos) {
        free(buffer);
        return -1;
    }
    ...
    if (header_len >= sizeof(type_str)) {
        free(buffer);
        return -1;
    }

    sscanf(buffer, "%15s", type_str);

    size_t size;
    sscanf(buffer + strlen(type_str) + 1, "%zu", &size);
